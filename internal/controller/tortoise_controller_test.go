...
func startController(ctx context.Context) func() {
	...
	tortoiseService, err := tortoise.New(mgr.GetClient(), recorder, 24, "Asia/Tokyo", 1000*time.Minute, "daily", false, nil)
	Expect(err).ShouldNot(HaveOccurred())
	cli, err := vpa.New(mgr.GetConfig(), recorder)
	Expect(err).ShouldNot(HaveOccurred())
	hpaS, err := hpa.New(mgr.GetClient(), recorder, 0.95, 90, 25, time.Hour, nil, 1000, 10000, 3, ".*-exclude-metric", 5*time.Minute, false, nil)
	Expect(err).ShouldNot(HaveOccurred())
