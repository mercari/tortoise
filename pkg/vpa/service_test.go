...
func TestVPAContainerResourcePolicy(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := New(fake.NewClientBuilder().WithRuntimeObjects(tt.initialVPA).Build(), record.NewFakeRecorder(10), 0.95, 90, 50, time.Hour)
			got, tortoise, err := c.UpdateVPAContainerResourcePolicy(tt.args.ctx, tt.args.tortoise, tt.args.now)
